-- Fix services RLS policies to work with serial number authentication
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'services' AND policyname = 'Users can view active services') THEN
        DROP POLICY "Users can view active services" ON public.services;
    END IF;
...
-- Create new policies that work with serial number authentication
-- Allow everyone to view active services
CREATE POLICY "Users can view active services" ON public.services
  FOR SELECT USING (is_active = true);

-- Allow clinic admins to manage their services (based on admin_id)
...
-- Allow system admins to manage all services
CREATE POLICY "Admins can manage all services" ON public.services
  FOR ALL USING (public.is_admin());

-- TEMPORARY: Allow all authenticated users to manage services (for testing)
-- This should be removed once proper clinic admin authentication is implemented
